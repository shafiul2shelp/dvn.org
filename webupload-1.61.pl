...
chmodx ($_) foreach( @{$commands{chmodx}});
logout();
...
sub process_cmd {
  my($cmd) = @_;
  push @{$commands{$1}}, $2 if($cmd =~ m/^%\s*([^ ]+?)\s+([^ ]+)/);
}
...
sub chmodx {
    my($file) = @_;
    print "Attempting to chmod file $file...";
    $ftp->site("chmod", "777", $file) or print_failure();
    print "\n";
}
...
sub translate_cygpath {
  my($cygpath) = @_;
  my($winpath);
  if($^O eq "MSWin32" and $ENV{CYGWIN}) {
...
=item * B<--config | -c> I<filename>

Name of config file containg FTP host settings

=item * B<--file | -f> I<filename|dirname>
