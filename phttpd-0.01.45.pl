#!/usr/bin/perl
# karakurt - pure perl httpd v0.01.45 (c) mahatma, GPLs

=head1 NAME

phttpd-0.01.45.pl - karakurt, small pure Perl httpd ([x]inetd or standalone).
...
karakurt, pure Perl httpd v0.01.45           (c) Dzianis Kahanovich, GPLs
...
sub print_{
my ($i,$n,$l)=(0,0,length($_[0]));
while($l>$i){
 select(undef,my $y=$vec_out,undef,$RANGE[9])&&
 (($n=send($OUT,substr($_[0],$i,$wbufsize),$wbufsize,$MSG_DONTWAIT))>0)
 ||return 0;
 $i+=$n;
 if($maxwbufsize && (my $wb=$wbufsize-$n)>0 && (my $wb1=$l-$i)>0){
  $wb=$wb1 if($wb1<$wb);
  if(($wb+=unpack('L',getsockopt($OUT,$SOL_SOCKET,$SO_SNDBUF)))<=$maxwbufsize){
   setsockopt($OUT,$SOL_SOCKET,$SO_SNDBUF,pack('L',$wb>>1));
   (($n=send($OUT,substr($_[0],$i,$maxwbufsize),$maxwbufsize,$MSG_DONTWAIT))<0)&&return 0;
   $i+=$n;
  }
 }
}
