#!/usr/bin/perl
# karakurt - pure perl httpd v0.01.36 (c) mahatma, GPLs

=head1 NAME

phttpd-0.01.36.pl - karakurt, small pure Perl httpd ([x]inetd or standalone).
...
karakurt, pure Perl httpd v0.01.36           (c) Dzianis Kahanovich, GPLs
...
for(@{$x2}){
 ...
 $x=~s/$y1//;
 $x1=~s/$y1/return ($x=ref($y2)) eq 'ARRAY'?translate($y2,$_[1]):$x eq 'CODE'?&$y2($_[1]):$y2/e if($x eq '')
}
}
...
my (%OPTIONS,@RANGE,@errors,$mainpid,$SERVER,$IN,$OUT,$vec_in,$vec_out,$rbufsize,$wbufsize,$filebuf,$log,$a);
my $tid='';
...
my ($rpos,$pos,$buf);
my ($MSG_PEEK,$MSG_DONTWAIT,$PF_INET,$SOCK_STREAM,$PROTO_TCP,$SOL_SOCKET,$SO_REUSEADDR,$SOMAXCONN,$SO_ERROR,$SO_SNDBUF,$SO_RCVBUF,$INADDR_ANY,$SO_LINGER,$SO_KEEPALIVE)=(2,64,2,1,6,1,2,512,4,7,8,pack('x4'),13,9);

my $ID='karakurt/0.01.36';
...
my %contents,%URIs,%options;
my @saveguid=($>,$));
my %methods=('GET'=>1,'POST'=>1,'HEAD'=>1);

sub httpd{

my ($content,@guid,$conf);
...
do $OPTIONS{'c'} if($conf=-M $OPTIONS{'c'});
 ...
 *xfork=sub {
  my $th;
  "$tid$$" eq $mainpid&&($th=threads->new(\&httpd,@_))&&$th->detach;
  $th
 }
) if(exists($OPTIONS{'th'}));

&socklib if(exists($OPTIONS{'l'}));
if(exists($OPTIONS{'L'})){
 open $log,">>$OPTIONS{'L'}";
...
while(1){
 ...
 if(exists($OPTIONS{'c'})&&$conf!=(my $x=-M $OPTIONS{'c'})){$conf=$x;do $OPTIONS{'c'}}
...
$OUT=$IN
 ...
 exit print qq($ID pure Perl httpd, (c) Dzianis Kahanovich, 2005-2006, GPLs
...
MAP_OK:
 ...
 EXEC_CGI:
  ...
  $contents{$file}=[$content=xeval("sub \{ $content\n\};"),@stat] if($mode==2);
  die $@ if($@);
  return $mode,$content if($RANGE[2]<4&&$tid==0);
...
if(defined($b)){
 ...
 if($RANGE[2]){
  ...
  $RANGE[2]=0 if($l eq 'close'||($l ne 'keep-alive'&&$ENV{'SERVER_PROTOCOL'} lt 'HTTP/1.1'));
 }
...
sub end{
 ...
 if($log){for(@errors,$!,$@){$_&&wlog('ERROR:',"<$_>")}}
 close($IN);
 close($OUT);
 close($SERVER);
 close($log);
