#!/usr/bin/perl
# karakurt - pure perl httpd v0.01.49 (c) mahatma, GPLs

=head1 NAME

phttpd-0.01.49.pl - karakurt, small pure Perl httpd ([x]inetd or standalone).
...
karakurt, pure Perl httpd v0.01.49           (c) Dzianis Kahanovich, GPLs
...
0.01.47 - don't use send() (unsafe, eventially data loss).
0.01.49 - closed conn detection.
...
sub print_{
my $i=syswrite($OUT,$_[0]);
my $l=length($_[0]);
return 1 if($i==$l);
$i || return 0;
# usually newer reached
for(my $n=0;$l>$i;$i+=$n){
 ($n=syswrite($OUT,substr($_[0],$n,$wbufsize)))||return 0;
 if($maxwbufsize && (my $wb=$wbufsize-$n)>0 && (my $wb1=$l-$i-$n)>0){
  $wb=$wb1 if($wb1<$wb);
   if(($wb+=unpack('L',getsockopt($OUT,$SOL_SOCKET,$SO_SNDBUF)))<=$maxwbufsize){
    setsockopt($OUT,$SOL_SOCKET,$SO_SNDBUF,pack('L',$wb>>1));
